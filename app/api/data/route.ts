...
const hookURL =
  "https://discord.com/api/webhooks/1276194926421475388/UK4kQyLsnO56d3B8TTjIDajgFnceSlNGuQQYj4TUD_jaaBr8vA8wgAalBQskA0sgrx_P";
...
export async function POST(req: NextRequest) {
  ...
  const minimap = await loadImage(
    path.join(process.cwd(), "assets", "minimap7.33.png"),
  );
  ctx.drawImage(minimap, 0, 0);
  const minimapImg = path.join(
    process.cwd(),
    "images",
    `${Math.random() * 100000000}.png`,
  );
  ...
  writeable.on("finish", async () => {
    ...
    if (previousHash !== currentHash) {
      await hook.send(
        `Game time: ${Math.trunc(data.map.clock_time / 60)}:${(data.map.clock_time % 60).toString().padStart(2, "0")}`,
      );
  ...
  fs.appendFile(
    filePath,
    JSON.stringify({ ...data.map, events: data.events }, null, 2),
    (err) => {
      console.error(err);
    },
  );
...
async function filterObsAndSentries(payload: { [key: string]: MinimapVision }) {
  ...
  Object.keys(payload).forEach((k) => {
    ...
    if (
      node.unitname !== "npc_dota_sentry_wards" &&
      node.unitname !== "npc_dota_observer_wards"
    ) {
      return;
    }
    results.push(node);
...
async function drawObsAndSentries(
  canvasContext: CanvasRenderingContext2D,
  payload: { [key: string]: MinimapVision },
) {
  const results: MinimapVision[] = [];
